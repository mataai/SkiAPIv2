...
export class GroupService {
    ...
    async getAll(userID: number, query?: any, includeStudents = false): Promise<Group[]> {
        const perms = await this.permsService.getPerms(userID);
        let output: Group[] = [];
        const filter = {
            where: {
                groupId: query.groupId ? In(query.groupId?.split(',') || []) : MoreThanOrEqual(0),
                levelId: query.levelId ? In(query.levelId?.split(',') || []) : MoreThanOrEqual(0),
                departementId: query.departementId ? In(query.departementId?.split(',') || []) : MoreThanOrEqual(0),
                teacherId: query.teacherId ? In(query.teacherId?.split(',') || []) : MoreThanOrEqual(0),
                day: query.day ? In(query.day?.split(',') || []) : MoreThanOrEqual(0),
                nbStudents: query.nbStudents ? In(query.nbStudents?.split(',') || []) : MoreThanOrEqual(0)
        ...
        if (await this.permsService.isAdmin(userID)) {
            ...
            if (perms2.length > 0) { //View Grp Dept
                ...
                depts = query.departementId ? depts.concat(depts, query.departementId.split(',')) : depts;
                filter.where.departementId = depts && depts.length > 0 ? In(depts) : MoreThanOrEqual(0);
                this.addToList(output, await this.groupsRepository.find(filter));
            }
            if (perms1.length > 0) { // View Grp Self
        ...
        return output.map(x => {
            ...
            includeStudents ? null : delete x.studentgroups;
            return x;
        })
